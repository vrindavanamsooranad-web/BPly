          ...
          <div className={isPublic ? 'opacity-40 pointer-events-none select-none' : ''}>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all disabled:opacity-50 disabled:bg-slate-50"
              />
              <button
                onClick={handleAddEmail}
                disabled={isPublic || saving}
                className="flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-lg transition-colors disabled:opacity-50 disabled:bg-slate-300"
              >
...
export default function History() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text('BLOOD PRESSURE REPORT', pw / 2, y, { align: 'center' });

      y += 10;
      
      // Patient Profile on the Left
      pdf.setFont('helvetica', 'bold'); pdf.setFontSize(12); pdf.setTextColor(15, 23, 42);
      pdf.text(userProfile?.name?.toUpperCase() || 'A S KRISHNAJITH', lm, y);
      
      y += 6;
      pdf.setFont('helvetica', 'normal'); pdf.setFontSize(9); pdf.setTextColor(100, 116, 139);
      const gender = userProfile?.gender?.toUpperCase() || 'MALE';
      const dob = userProfile?.dob || '2009-03-18';
      pdf.text(`AGE: ${patientAge}  |  GENDER: ${gender}  |  DOB: ${dob}`, lm, y);
      
      // Report Date Range on the right
      pdf.text(`DATE: ${reportDateRange}`, pw - lm, y, { align: 'right' });
